...
def read_psi4_dir(
    ...
    for ell in range(ell_min, ell_max + 1):
        ...
        time_data, indices = np.unique(data[:, 0], return_index=True)
        data = data[indices]  # sort data accordingly

        if n_times == -1:
            n_times = len(time_data)
        elif n_times != len(time_data):
            raise ValueError(
                f"Inconsistent times for l={ell}. Expected {n_times} time points, got {len(time_data)}."
            )
...
def psi4_ffi_to_strain(
    ...
    :raises IOError: Error reading the PSI4 data or writing strain data.
    """
    ...
    freq_at_t0 = fit_quadratic_to_omega_to_find_absomega_at_t0(
        time_arr, ell2em2_wave[3], ext_rad, interval
    )
    freq_cutoff = freq_at_t0 * cutoff_factor
...
def find_file_for_l(data_dir: str, ell: int) -> str:
    ...
    :param data_dir: The directory to search for the file.
    :param ell: l mode to search for.
    :return: Path to the found file.
    :raises FileNotFoundError: If no file matching the ell pattern is found in the specified directory.
    """
    expected_pattern = f"_l{ell}-"
    for filename in os.listdir(data_dir):
        if expected_pattern in filename:
            return os.path.join(data_dir, filename)
    raise FileNotFoundError(
        f"No file matching the expected pattern '{expected_pattern}' found in directory '{data_dir}'."
    )
...
def psi4_phase_and_amplitude(
    ...
    omega = first_time_derivative(time, cum_phase)
    return time, amplitudes, cum_phase, omega


def fit_quadratic_to_omega_to_find_absomega_at_t0(
    time: NDArray[np.float64],
    data: NDArray[np.float64],
    ext_rad: float,
    interval: float,
    ...
    :param ext_rad: The extraction radius used to define the starting point of the time interval.
    :param interval: The length of the time interval from the starting point for data sampling.
    :param verbose: If True, prints additional debug information about the quadratic fit.
